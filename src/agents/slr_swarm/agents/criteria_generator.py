...
async def run_criteria_generator(idea: str, research_field: str = "") -> CriteriaGenerationResult:
    ...
    try:
        # Same reasoning as scope_optimizer.py: this proposes concrete
        # inclusion/exclusion rules the researcher will read as a checklist,
        # not prose -- lower temperature so re-running on the same topic
        # doesn't produce a materially different checklist each time.
        ...
        # This used to fabricate 4 fixed inclusion/exclusion criteria (only
        # the first line templated with `idea`) and return HTTP 200 --
        # indistinguishable from a real suggestion, so a transient provider
        # failure silently handed the researcher generic boilerplate as if it
        # were tailored to their topic. The frontend (ResearchSetupTab.jsx)
        # already shows a clear error message on a non-200 response; letting
        # the failure propagate is all that was missing.
        logger.warning("LLM call failed in run_criteria_generator: %s", exc)
        raise
