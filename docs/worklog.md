...
| Nam Hải | Upload video demo lên YouTube, chốt `docs/pitch-deck.pdf` (đổi tên từ `slide.pdf` → `pitch_deck.pdf` → `pitch-deck.pdf` cho khớp naming convention BTC yêu cầu) làm bản slide chính thức (deliverable #7), xóa 2 bản slide cũ trùng lặp (`presentation/pitch_deck.pdf`, `.pptx`) + 1 file rác Office lock, cập nhật `docs/video-demo.md` với link YouTube thật, bổ sung deliverable #7 vào bảng README, chuyển link deliverable #8 sang `docs/journal.md` đúng vị trí yêu cầu | ✅ Done | `docs/video-demo.md`, `docs/pitch-deck.pdf`, `README.md` | - |
