...
export default function UploadTab({ selectedPapers, setActiveTab, darkMode }) {
  const [uploadedFiles, setUploadedFiles] = useState(selectedPapers);
  ...
  const handleAddUrl = (e) => {
    ...
    setUploadedFiles([newPaper, ...uploadedFiles]);
    setPastedUrl('');
  };
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto py-4">
      ...
      {/* Drag & Drop Card */}
      <div className={`p-8 md:p-12 text-center space-y-4 border-2 border-dashed rounded-3xl transition-colors ${
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 font-medium">Hỗ trợ các định dạng .PDF, .CSV, .TXT (Tối đa 50MB/file)</p>
        </div>
        <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-2xl text-xs shadow-md transition-all">
      ...
      <div className={`p-6 md:p-8 rounded-3xl border space-y-5 ${
        ...
        <div className="flex items-center justify-between border-b pb-4 border-slate-100 dark:border-slate-800">
          <h3 className="font-bold text-base flex items-center gap-2">
            ...
            <span>Danh sách Tài liệu Nguồn ({uploadedFiles.length} bài)</span>
          </h3>
