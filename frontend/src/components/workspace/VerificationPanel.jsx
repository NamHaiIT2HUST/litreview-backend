      ...
      {activeCitation ? (
        <div className="space-y-5 text-sm">
          <div>
            ...
            <h4 className={`font-extrabold text-base mt-3 leading-snug ${darkMode ? 'text-white' : 'text-slate-900'}`}>
              {activeCitation.title}
            </h4>
          </div>

          <div className="space-y-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            {activeCitation.authors && <p><strong>Tác giả:</strong> {activeCitation.authors}</p>}
            {(activeCitation.journal || activeCitation.year) && <p><strong>Nguồn:</strong> {activeCitation.journal} {activeCitation.year ? `(${activeCitation.year})` : ''}</p>}
            <p><strong>Trang PDF:</strong> {activeCitation.source_page_display ?? 'N/A'}</p>
            <p><strong>Raw page chars:</strong> {activeCitation.source_char_start ?? '?'}–{activeCitation.source_char_end ?? '?'}</p>
          </div>
          ...
          {activeCitation.url && activeCitation.url !== '#' && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-xs shadow-md"
            >
        ...
        <p className="text-slate-400 text-xs italic">
          Chạy synthesis rồi click marker [1], [2] trong tổng quan để xem đúng quote, trang và offset đã được grounding.
        </p>
      )}
    </div>
  );
